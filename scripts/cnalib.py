...
The input will be read from the file CUF.
...
try :
    ...
    if sys.version_info < (3, 5, 2) :
        print ('\n\nWARNING: This tool requires Python v.3.5.2 at least!!\n')
...
class Cna :
    def __init__ (self, opt) :
        ...
        if self.opt['stb'] and self.opt['coverability'] :
            raise Exception ('Stubborn reduction only works with deadlock checking')
        ...
        try :
            ...
            raise Exception ("input file: %s" % e)
        except Exception as e:
...
#       st3: post (pre (e) U cont (e)) without blacks subset of {e}
    ...
    def solve (self) :
        ...
        # errors
        if s != 10 :
            raise Exception ('Minisat terminated with unknown exit status %d' % s)
