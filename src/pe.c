	...
	for (i = c->post.deg - 1; i >= 0; i--) {
		...
		for (j = ep->hist.deg - 1; j >= 0; j--) {
			...
			for (k = hi->nod.deg - 1; k >= 0; k--) {
				...
				/* build the list of conditions generated by
				 ...
				 * consumes at least one condition in lst, we
...
static void _pe_pcomb_init (struct cond * c, struct trans * t)
{
	...
	for (i = t->pre.deg - 1; i >= 0; i--) {
		...
		pe.pcomb.tab[pe.pcomb.size].contidx = -1;
		pe.pcomb.size++;
	}
...
static void _pe_pcomb_next (void)
{
	...
	while (1) {
		...
		/* explore the next event in the context of this condition */
		pe.pcomb.tab[idx].contidx++;

		/* if not all events are explored, we are done */
		c = (struct cond *) pe.pcomb.tab[idx].n->node;
		if (pe.pcomb.tab[idx].contidx < c->cont.deg) return;

		/* otherwise, move to the next pointer in the last list */
		pe.pcomb.tab[idx].n = pe.pcomb.tab[idx].n->next;
		pe.pcomb.tab[idx].contidx = -1;
...
static void _pe_comb_init (void)
{
	 ...
	 * the list */
	ASSERT (pe.comb.h->e == pe.comb.c->pre);
	...
	for (i = 0; i < pe.pcomb.size; i++) {
		...
		/* we get a pointer to the condition and to the event
		 * generating or reading it */
		cp = pe.pcomb.tab[i].n->node;
		ASSERT (pe.pcomb.tab[i].contidx >= -1);
		ASSERT (pe.pcomb.tab[i].contidx < cp->cont.deg);
		if (pe.pcomb.tab[i].contidx == -1) {
			e = cp->pre;
		} else {
			e = dg_i (struct event,
				cp->cont.adj[pe.pcomb.tab[i].contidx], cont);
		}

		/* if the event has been already marked, we find it and append
		 * this condition to the list of conditions generated by it */
		if (e->m == mark) {
			for (j = pe.comb.size - 1; j >= 0; j--) {
				if (pe.comb.tab[j].e == e) break;
			}
			if (j >= 0) {
				nl_push (&pe.comb.tab[j].conds, cp);
			} else {
				ASSERT (pe.comb.h->e == e);
...
static struct event * _pe_already (struct trans * t)
{
	int i;
	struct nl * n;
	struct cond * c;
	...
	/* given conditions pe.comb.c and all conditions in pe.pcomb.tab, check
	 * if there is already an event labeled with t and using exactly these
	 * conditions as preset + context */
	ASSERT (t);
	ASSERT (t->pre.deg + t->cont.deg == pe.pcomb.size + 1);

	/* for each event labeling t */
	for (n = t->events; n; n = n->next) {
		e = (struct event *) n->node;
		ASSERT (e->pre.deg == t->pre.deg);
		ASSERT (e->cont.deg == t->cont.deg);

		/* check whether c is in its preset or context */
		if (pe.comb.cispre) {
			if (! dg_test (&e->pre, (void *) &pe.comb.c->pre)) 
					continue;
		} else {
			if (! dg_test (&e->cont, &pe.comb.c->cont)) continue;
		}

		for (i = pe.pcomb.size - 1; i >= 0; i--) {
			c = pe.pcomb.tab[i].n->node;
			if (dg_test (&e->pre, (void *) &c->pre)) continue;
			if (! dg_test (&e->cont, &c->cont)) break;
		}
		BREAK (i < 0);
		if (i < 0) return e;
	}
...
static struct event * _pe_new_event (struct trans * t)
{
	...
	/* check if this occurence of t is already present in the unfolding */
	e = _pe_already (t);
	if (e) return e;
