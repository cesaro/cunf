	...
	for (i = c->post.deg - 1; i >= 0; i--) {
		...
		for (j = ep->hist.deg - 1; j >= 0; j--) {
			...
			for (k = hi->nod.deg - 1; k >= 0; k--) {
				...
				/* and for each condition in preset(ep) or in
				 * context(ep) generated by shi */
				for (l = ep->pre.deg - 1; l >= 0; l--) {
					ci = dg_i (struct cond, ep->pre.adj[l],
							pre);
					if (ci->pre != shi->e) continue;

					/* if h is in conflict with some shi,
					 * we don't have a new history for ep
					 */
					if (h_conflict2 (h, c, shi, ci)) break;
				}
				if (l >= 0) break;
...
static struct h * _pe_update_build (void)
{
	...
	if (pe.comb.ep == 0) {
		...
		for (i = pe.comb.size - 1; i >= 0; i--) {
			if (pe.comb.tab[i].c->causalm != m) break;
		}
		if (i < 0) {
			e = pe.comb.ep;
		} else {
...
static int _pe_comb_explore (int idx)
{
	...
	for (i = pe.comb.tab[idx].e->hist.deg - 1; i >= 0; i--) {
		...
		if (h_conflict2 (pe.comb.h, pe.comb.c, h, pe.comb.tab[idx].c)) {
			continue;
		}

		/* and also with all histories below index idx */
		for (j = 0; j < idx; j++) {
			if (h_conflict2 (pe.comb.tab[j].h, pe.comb.tab[j].c,
					h, pe.comb.tab[idx].c)) break;
		}
